...
grep -E '^\|[[:space:]]*[0-9]+[[:space:]]*\|[[:space:]]*FEAT-[0-9]+[[:space:]]*\|' "$DOC" | while IFS= read -r line; do
	...
	if git show-ref --verify --quiet "refs/heads/$BRANCH_NAME"; then
		# Safety: refuse to delete branch with unmerged commits
		UNMERGED=$(git log --oneline "main..$BRANCH_NAME" 2>/dev/null | wc -l | tr -d ' ')
		if [ "$UNMERGED" -gt 0 ]; then
			echo "Error: Branch '$BRANCH_NAME' has $UNMERGED unmerged commit(s). Skipping."
			echo "  Use 'git branch -D $BRANCH_NAME' manually if you want to discard them."
			continue
		fi
