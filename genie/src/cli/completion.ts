...
function renderBash(): string {
  return `# bash completion for genie
_genie() {
...
function renderFish(): string {
  const lines = [
    '# fish completion for genie',
    ...rootCommands.map((command) => `complete -c genie -n "__fish_use_subcommand" -a "${command}"`),
    `complete -c genie -f -s h -l help`,
    `complete -c genie -f -l version`,
    `complete -c genie -f -l json`,
    `complete -c genie -f -l plain`,
    `complete -c genie -f -l no-color`,
    `complete -c genie -f -s q -l quiet`,
    ...
    ...providersSubcommands.map((command) => `complete -c genie -n "__fish_seen_subcommand_from providers" -a "${command}"`),
    ...configSubcommands.map((command) => `complete -c genie -n "__fish_seen_subcommand_from config" -a "${command}"`),
    ...presetsSubcommands.map((command) => `complete -c genie -n "__fish_seen_subcommand_from presets" -a "${command}"`),
    ...helpTopics.map((topic) => `complete -c genie -n "__fish_seen_subcommand_from help" -a "${topic}"`),
    ...shells.map((shell) => `complete -c genie -n "__fish_seen_subcommand_from completion" -a "${shell}"`),
  ]
