...
Repository scope: this file applies to the entire `yardmaster` repository.
...
- Architecture truth: prompt commands and `yardmaster review` are ACP-first flows; `yardmaster providers doctor` remains CLI/doctor-oriented. Do not document legacy spawn-adapter architecture as the current design.
- Verification parity with CI (from `yardmaster/`): `bun install --frozen-lockfile`, `bun run typecheck`, `bun run test`, `bun run build`, `bun run test:critical-path`.
...
- [Dispatch skill](.claude/skills/yardmaster-dispatch/SKILL.md) — provider strengths and routing guidance.
