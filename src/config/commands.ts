...
import { defaultConfig, yardmasterConfigSchema, type YardmasterConfig } from './schema.js'
...
function getByPath(config: YardmasterConfig, key?: string): unknown {
...
function applyConfigValue(current: YardmasterConfig, key: ConfigKey, parsed: unknown): YardmasterConfig {
  const next: YardmasterConfig = structuredClone(current)
  const segments = key.split('.')
  ...
  return yardmasterConfigSchema.parse({
    ...defaultConfig,
    ...next,
  })
}
...
export async function configSet(key: string, value: string): Promise<YardmasterConfig> {
  if (!isConfigKey(key)) {
    throw new UsageError(`Unknown config key '${key}'`)
  }
...
export async function previewConfigSet(key: string, value: string): Promise<YardmasterConfig> {
  if (!isConfigKey(key)) {
    throw new UsageError(`Unknown config key '${key}'`)
  }
...
export async function configInit(): Promise<YardmasterConfig> {
  return initUserConfig()
}

export async function previewConfigInit(): Promise<{ path: string; exists: boolean; config: YardmasterConfig }> {
