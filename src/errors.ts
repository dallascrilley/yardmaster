...
export const ACP_AUTH_ERROR_CODE = -32000
...
export function getExitCode(error: unknown): number {
   ...
   if (error instanceof AcpProtocolError) {
      if (error.code === ACP_AUTH_ERROR_CODE) return 3
      return 1
   }
...
export function formatCliError(error: unknown): string {
   ...
   if (error instanceof AuthConfigurationError) {
      return withNextSteps(error.message, [
         'Run `yardmaster providers doctor` to see which auth check is failing.',
   ...
   if (error instanceof AcpProtocolError) {
      return withNextSteps(`ACP protocol error from ${error.providerId}: ${error.message}`, [
         'Run `yardmaster providers doctor` to check adapter health.',
         'Retry with `--provider <id>` to try a different provider.',
      ])
   }

   if (error instanceof Error) {
      return withNextSteps(error.message, ['Run `yardmaster help` if you need to verify command syntax before retrying.'])
   }
