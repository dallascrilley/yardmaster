...
export async function handleConfigGetCommand(parsed: Extract<ParsedCommand, { kind: 'config-get' }>): Promise<void> {
  ...
  writeVerbose(parsed.globals, `[yardmaster] command=config-get key=${parsed.key ?? 'all'}`)
}
...
export async function handleConfigSetCommand(parsed: Extract<ParsedCommand, { kind: 'config-set' }>): Promise<void> {
  ...
  writeVerbose(parsed.globals, `[yardmaster] command=config-set key=${parsed.key} dryRun=${String(parsed.safety.dryRun)}`)
}
...
export async function handleConfigInitCommand(
  ...
  writeVerbose(parsed.globals, `[yardmaster] command=config-init dryRun=${String(decision === 'dry-run')} existed=${String(preview.exists)}`)
}
...
export async function handleConfigPathCommand(parsed: Extract<ParsedCommand, { kind: 'config-path' }>): Promise<void> {
  ...
  writeVerbose(parsed.globals, '[yardmaster] command=config-path')
}
