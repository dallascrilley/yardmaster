...
export async function handlePresetsListCommand(parsed: Extract<ParsedCommand, { kind: 'presets-list' }>): Promise<void> {
  ...
  writeVerbose(parsed.globals, `[yardmaster] command=presets-list count=${Object.keys(value.named).length}`)
}
...
export async function handlePresetsGetCommand(parsed: Extract<ParsedCommand, { kind: 'presets-get' }>): Promise<void> {
  ...
  writeConfigValue(value)
  writeVerbose(parsed.globals, `[yardmaster] command=presets-get name=${parsed.name}`)
}
...
export async function handlePresetsSetCommand(
  ...
  writeVerbose(parsed.globals, `[yardmaster] command=presets-set name=${result.name} dryRun=${String(decision === 'dry-run')} replaced=${String(result.replaced)}`)
}
...
export async function handlePresetsDeleteCommand(
  ...
  writeVerbose(parsed.globals, `[yardmaster] command=presets-delete name=${result.deleted} dryRun=${String(decision === 'dry-run')}`)
}
...
export async function handlePresetsUseCommand(parsed: Extract<ParsedCommand, { kind: 'presets-use' }>): Promise<void> {
  ...
  writeVerbose(parsed.globals, `[yardmaster] command=presets-use default=${result.default} dryRun=${String(parsed.safety.dryRun)}`)
}
