...
export function parseDebugArgs(tokens: string[]): ParsedCommand {
  ...
  for (let index = 0; index < tokens.length; index += 1) {
    ...
    throw new UsageError(`Unexpected positional argument '${token}'. Pipe terminal output into yardmaster debug instead.`)
  }
...
export function parseCommitArgs(tokens: string[]): ParsedCommand {
  ...
  for (let index = 0; index < tokens.length; index += 1) {
    ...
    throw new UsageError(`Unexpected positional argument '${token}'. yardmaster commit reads staged git changes directly.`)
  }

  if (globals.version) return { kind: 'version' }
  if (globals.json) {
    throw new UsageError('--json is not supported for yardmaster commit')
  }
