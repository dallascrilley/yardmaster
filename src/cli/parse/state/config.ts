...
export function parseConfigArgs(tokens: string[]): ParsedCommand {
  ...
  if (subcommand === 'get') {
    if (positional.length > 1) {
      throw new UsageError('Usage: yardmaster config get [key]')
    }
  ...
  if (subcommand === 'set') {
    if (positional.length < 2) {
      throw new UsageError('Usage: yardmaster config set <key> <value>')
    }
  ...
  if (subcommand === 'init') {
    if (positional.length > 0) {
      throw new UsageError('Usage: yardmaster config init')
    }
  ...
  if (positional.length > 0) {
    throw new UsageError('Usage: yardmaster config path [--json]')
  }
