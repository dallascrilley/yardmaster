...
export const YARDMASTER_PI_BACKEND_ENV = 'YARDMASTER_PI_BACKEND'

export const PROVIDER_ALIAS_REGISTRY = {
  pi: { modelEnvVar: 'YARDMASTER_PI_MODEL' as const },
} satisfies Record<ProviderAliasId, { modelEnvVar?: string }>

export function resolvePiBackendProvider(): ProviderId {
  const raw = process.env[YARDMASTER_PI_BACKEND_ENV]?.trim().toLowerCase()
