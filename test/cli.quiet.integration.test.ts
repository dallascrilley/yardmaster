...
describe('cli quiet integration', () => {
  ...
  it('suppresses status-only success output for mutating config and presets commands', () => {
    const homeDir = createTempDir('yardmaster-quiet-home')
    ...
    const configInit = spawnSync('bun', ['src/bin/yardmaster.ts', 'config', 'init', '--quiet'], { cwd, encoding: 'utf8', env })
    expect(configInit.status).toBe(0)
    expect(configInit.stdout).toBe('')
    expect(configInit.stderr).toBe('')

    const configSet = spawnSync('bun', ['src/bin/yardmaster.ts', 'config', 'set', 'mode.default', 'default', '--quiet'], {
      cwd,
      encoding: 'utf8',
      env,
    })
    ...
    const presetSet = spawnSync(
      'bun',
      ['src/bin/yardmaster.ts', 'presets', 'set', 'quiet-mode', '--provider', 'codex', '--quiet'],
    ...
    const presetUse = spawnSync('bun', ['src/bin/yardmaster.ts', 'presets', 'use', 'quiet-mode', '--quiet'], {
      cwd,
      encoding: 'utf8',
      env,
    })
    ...
    const presetDelete = spawnSync('bun', ['src/bin/yardmaster.ts', 'presets', 'delete', 'quiet-mode', '--quiet', '--force'], {
      cwd,
      encoding: 'utf8',
      env,
    })
