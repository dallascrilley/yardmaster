...
import { createYardmasterClient } from '../src/acp/host-handlers.js'
import type { StreamEvent } from '../src/acp/types.js'

const thisFile = fileURLToPath(import.meta.url)

describe('createYardmasterClient', () => {
  describe('readTextFile', () => {
    it('reads the contents of a real file', async () => {
      const events: StreamEvent[] = []
      const client = createYardmasterClient({
        workspace: '/tmp',
        trustMode: 'trust',
      ...
      expect(result.content).toContain('createYardmasterClient')
    })
  ...
  describe('writeTextFile', () => {
    it('throws in default mode', async () => {
      ...
      await expect(
        client.writeTextFile!({
          path: '/tmp/yardmaster-test-write.txt',
          content: 'hello',
          sessionId: 'test-session',
        }),
    ...
    it('creates parent directories when writing to a nested path', async () => {
      const testWorkspace = `/tmp/yardmaster-test-workspace-${Date.now()}`
