...
describe('update command', () => {
  ...
  it('formats dry-run output without invoking commands', () => {
    const preview = previewUpdateCommand({ packageRoot: '/tmp/yardmaster' })
  ...
  it('resolves package root from src and dist module locations', () => {
    const fromSrc = resolveCliPackageRoot('file:///tmp/yardmaster/src/update/command.ts')
    const fromDist = resolveCliPackageRoot('file:///tmp/yardmaster/dist/update/command.js')
    expect(fromSrc).toBe('/tmp/yardmaster')
    expect(fromDist).toBe('/tmp/yardmaster')
  })
})
