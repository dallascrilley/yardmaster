...
const sourceCliPath = join(projectRoot, 'src', 'bin', 'yardmaster.ts')
...
function createStagedRepo(): string {
  const dir = mkdtempSync(join(tmpdir(), 'yardmaster-smoke-commit-'))
...
describe('smoke: commit', () => {
  ...
  describe.each(piSmokeBackends)('pi alias (YARDMASTER_PI_BACKEND=%s)', (backend) => {
    it('generates a non-empty commit message', async (ctx) => {
      ...
      const result = await spawnWithTimeout(
        ...
        { cwd: repoDir, timeoutMs: 55_000, env: { ...process.env, YARDMASTER_PI_BACKEND: backend } },
      )
  ...
  describe.each(piSmokeBackends)('pi alias (YARDMASTER_PI_BACKEND=%s)', (backend) => {
    it('returns a response containing "hello"', async (ctx) => {
      ...
      const result = await spawnWithTimeout(
        ...
        { cwd: projectRoot, timeoutMs: 55_000, env: { ...process.env, YARDMASTER_PI_BACKEND: backend } },
      )
