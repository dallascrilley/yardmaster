...
import { defaultConfig, type YardmasterConfig } from '../src/config/schema.js'
import { loadConfig, loadUserConfig, resolveUserConfigPath, saveConfig, updateConfig } from '../src/config/store.js'

function createTempHome(): string {
  const home = join(tmpdir(), `yardmaster-config-${randomUUID()}`)
...
describe('config store', () => {
  ...
  it('saves and loads config with persisted metadata', async () => {
    ...
    expect(loaded).toHaveProperty('_meta.schema', 'yardmaster-config-v1')
  })
  ...
  it('merges partial updates and preserves other sections', async () => {
    ...
    const initial: YardmasterConfig = {
      ...defaultConfig,
      model: {
        byProvider: {
          claude: 'claude-3',
