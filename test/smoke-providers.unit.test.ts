...
describe('smoke-providers', () => {
  const original = process.env.YARDMASTER_SMOKE_PROVIDERS

  afterEach(() => {
    if (original === undefined) {
      delete process.env.YARDMASTER_SMOKE_PROVIDERS
    } else {
      process.env.YARDMASTER_SMOKE_PROVIDERS = original
    }
  })
  ...
  it('defaults when env is only whitespace', () => {
    process.env.YARDMASTER_SMOKE_PROVIDERS = '  '
    expect(resolveSmokeProviders()).toEqual([...defaultSmokeProviders])
  })

  it('parses comma-separated provider ids', () => {
    process.env.YARDMASTER_SMOKE_PROVIDERS = ' gemini , claude '
