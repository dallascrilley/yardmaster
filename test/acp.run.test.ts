...
describe('runViaAcp', () => {
   ...
   it('preserves alias-model metadata for explicit alias providers', async () => {
      const executeAcpFallback = await getExecuteAcpFallback()
      const previousBackend = process.env.YARDMASTER_PI_BACKEND
      const previousModel = process.env.YARDMASTER_PI_MODEL
      process.env.YARDMASTER_PI_BACKEND = 'gemini'
      process.env.YARDMASTER_PI_MODEL = 'pi-gemini-pro'

      try {
         await runViaAcp({ prompt: 'hello', config: defaultConfig, provider: 'pi' })
      } finally {
         if (previousBackend === undefined) delete process.env.YARDMASTER_PI_BACKEND
         else process.env.YARDMASTER_PI_BACKEND = previousBackend
         if (previousModel === undefined) delete process.env.YARDMASTER_PI_MODEL
         else process.env.YARDMASTER_PI_MODEL = previousModel
      }
