...
describe('request normalization', () => {
  ...
  it('merges pi alias model when the same canonical provider appears as direct CLI later in the plan', () => {
    const prev = process.env.YARDMASTER_PI_MODEL
    const prevB = process.env.YARDMASTER_PI_BACKEND
    delete process.env.YARDMASTER_PI_BACKEND
    process.env.YARDMASTER_PI_MODEL = 'from-pi-env'
    try {
      const cfg = mergeConfig(defaultConfig, {
        provider: {
          default: 'codex',
          fallbackOrder: ['pi', 'gemini', 'claude'],
        },
      })
      const { slots } = resolveProviderExecutionPlan(cfg, undefined, false)
      expect(slots.find((s) => s.provider === 'gemini')).toMatchObject({
        provider: 'gemini',
        aliasModel: 'from-pi-env',
      })
    } finally {
      if (prev === undefined) delete process.env.YARDMASTER_PI_MODEL
      else process.env.YARDMASTER_PI_MODEL = prev
      if (prevB === undefined) delete process.env.YARDMASTER_PI_BACKEND
      else process.env.YARDMASTER_PI_BACKEND = prevB
    }
  })

  it('dedupes pi and codex when YARDMASTER_PI_BACKEND is codex', () => {
    const prevB = process.env.YARDMASTER_PI_BACKEND
    const prevM = process.env.YARDMASTER_PI_MODEL
    process.env.YARDMASTER_PI_BACKEND = 'codex'
    process.env.YARDMASTER_PI_MODEL = 'pi-codex-model'
    try {
      const cfg = mergeConfig(defaultConfig, {
        provider: {
          default: 'pi',
      ...
      const { slots } = resolveProviderExecutionPlan(cfg, undefined, false)
      expect(slots.map((s) => s.provider)).toEqual(['codex', 'claude'])
      expect(slots.filter((s) => s.provider === 'codex')).toHaveLength(1)
      expect(slots[0].aliasModel).toBe('pi-codex-model')
    } finally {
      if (prevB === undefined) delete process.env.YARDMASTER_PI_BACKEND
      else process.env.YARDMASTER_PI_BACKEND = prevB
      if (prevM === undefined) delete process.env.YARDMASTER_PI_MODEL
      else process.env.YARDMASTER_PI_MODEL = prevM
    }
  })
